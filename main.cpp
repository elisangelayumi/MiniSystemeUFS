...
/* Le tableau suivant est utilise par le parseur de commandes et l'interpreteur */
...
int main (int argc, char **argv)
{
	...
	//Creation de la variable representant le disque virtuel
...
int fonctionExecute(const string& p_commande, DisqueVirtuel *p_disqueVirtuel){
	...
	int retour = -1;
    // Decouper la commande en plusieurs chaines
	...
	for (size_t index = 0; index < 5; index++) {
		...
		//Recherche de la bonne commande a executer
		if (TableDesFonctions[index].StrCommande == v_splitCommande[0]) {

			//Nombre d'arguments ne correspondant pas a la commande
			if (TableDesFonctions[index].nArguments != (v_splitCommande.size() - 1)) {
				...
				//Appel de la commande en lui passant en parametre l'argument si necessaire
				switch (index){
